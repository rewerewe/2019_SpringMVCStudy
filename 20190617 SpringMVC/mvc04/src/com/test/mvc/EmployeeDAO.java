...
public class EmployeeDAO implements IEmployeeDAO
{
	...
	public ArrayList<Region> regionList() throws SQLException
	{
		...
		try
		{
			sql = "SELECT REGIONID, REGIONNAME FROM REGION ";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
	...
	public ArrayList<Position> positionList() throws SQLException
	{
		...
		try
		{
			sql = "SELECT POSITIONID, POSITIONNAME, MINBASICPAY FROM POSITION ";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			...
			while (rs.next())
			{
				...
				dto.setPositionId(rs.getString("POSITIONID"));
				dto.setPositionName(rs.getString("POSITIONNAME"));
				dto.setPositionName(rs.getString("MINBASICPAY"));
	...
	public int employeeAdd(Employee employee) throws SQLException
	{
		...
		try
		{
			sql  = "INSERT INTO EMPLOYEE( EMPLOYEEID, NAME, SSN1, SSN2										";
			sql += "					, BIRTHDAY, LUNAR													";
			sql += "                    , TELEPHONE, DEPARTMENTID, POSITIONID, REGIONID, BASICPAY, EXTRAPAY)";
			sql += "VALUES (EMPLOYEESEQ.NEXTVAL, ? , ? , CRYPTPACK.ENCRYPT( ? , ?)							";
